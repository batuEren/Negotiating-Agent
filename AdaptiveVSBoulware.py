...
def print_evaluation(df: pd.DataFrame, scenario_name: str):
    ...
    print(
        uw[["buyer_utility_mean", "seller_utility_mean",
            "social_welfare_mean", "buyer_advantage_mean"]]
        .sort_values("buyer_advantage_mean", ascending=False).to_string()
    )
...
def plot_utility_space(scenario_fn, df: pd.DataFrame,
    ...
    if pareto:
        ...
        ax.plot(px, py, color="steelblue", linewidth=2.0, zorder=3, label="Pareto frontier")
        ax.scatter(px, py, s=35, color="steelblue", zorder=4)

    # Nash point
    if nash:
        ax.scatter([nash[1]], [nash[2]], s=280, marker="D",
    ...
    for combo in df["combo"].unique():
        ...
        ax.scatter(grp["buyer_utility"], grp["seller_utility"],
                   s=60, alpha=0.70, color=COLORS.get(combo, "#888"),
                   label=f"{COMBO_SHORT[combo]}", zorder=5)
...
def plot_concession_paths(scenario_fn, scenario_name: str, n_steps: int = 30):
    ...
    for ax, combo in zip(axes, combos):
        ...
        if trace:
            ...
            ax.scatter(bu_vals, su_vals,
                            c=range(n_pts), cmap="plasma", s=35, zorder=4,
                            vmin=0, vmax=max(n_pts - 1, 1))
    ...
    fig.suptitle(f"Concession Paths — {scenario_name}\n"
    ...
    for ax, (sname, df) in zip(axes, zip(scenario_names, all_dfs)):
        ...
        for combo in df["combo"].unique():
            ...
            ax.scatter(grp["buyer_advantage"], grp["social_welfare"],
                       s=30, alpha=0.45, color=color, zorder=3)
            # Mean marker
            mx, my = grp["buyer_advantage"].mean(), grp["social_welfare"].mean()
            ax.scatter([mx], [my], s=200, marker="D", color=color,
        ...
        ax.text(0.98, 0.02, "High self\nLow joint", transform=ax.transAxes,
                ha="right", va="bottom", fontsize=7, color="gray")
        ax.text(0.98, 0.98, "High self\nHigh joint", transform=ax.transAxes,
                ha="right", va="top", fontsize=7, color="green")

    fig.suptitle("Self-interest vs Joint Quality Trade-off\n"
...
def plot_radar_summary(all_dfs: list[pd.DataFrame], scenario_names: list[str]):
    ...
    for ax, (sname, df) in zip(axes, zip(scenario_names, all_dfs)):
        ...
        for combo in df["combo"].unique():
            ...
            combo_metrics[combo] = {
                ...
                "social_welfare":   grp["social_welfare"].mean() / 2,  # normalise to [0,1]
        ...
        for combo, vals in combo_metrics.items():
            normed = [
                (vals[k] - mins[k]) / (maxs[k] - mins[k] + 1e-9)
                for k in all_keys
            ]
            normed += normed[:1]
            color = COLORS.get(combo, "#888")
            ax.plot(angles, normed, color=color, linewidth=2,
                    label=COMBO_SHORT[combo])
    ...
    fig.suptitle("Performance Radar — All Metrics\n"
